...
import { 
  Server, Layout, Bot, QrCode, FileSpreadsheet, Lightbulb, Check 
} from 'lucide-react';
          ...
          {services.map((service) => (
            <div key={service.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 overflow-hidden hover:shadow-2xl transition-all duration-300 flex flex-col">
              ...
              <div className="p-6 bg-gray-50 dark:bg-gray-900/50 border-t border-gray-100 dark:border-gray-700">
                <Link 
                  to="/contact" 
                  className="block w-full text-center py-2.5 rounded-lg border border-brand-600 text-brand-600 dark:text-brand-400 dark:border-brand-500 hover:bg-brand-600 hover:text-white dark:hover:bg-brand-500 transition-colors font-semibold"
                >
                  {t.common.contactUs}
                </Link>
              </div>
